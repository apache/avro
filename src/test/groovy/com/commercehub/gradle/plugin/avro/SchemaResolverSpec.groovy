...
class SchemaResolverSpec extends Specification {
    ...
    def "Duplicate record definition succeeds if definition identical"() {
        ...
        def files = resourceNames.collect { new File("src/test/resources/com/commercehub/gradle/plugin/avro/duplicate/${it}") }
    ...
    def "Duplicate record definition in single file fails with clear error"() {
        given:
        def file = new File("src/test/resources/com/commercehub/gradle/plugin/avro/duplicate/duplicateInSingleFile.avsc")
