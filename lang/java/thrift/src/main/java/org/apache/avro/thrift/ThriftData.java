...
public class ThriftData extends GenericData {
  ...
  private Schema getSchema(FieldValueMetaData f) {
    return getSchema(f, true);
  }

  private Schema getSchema(FieldValueMetaData f, boolean defaultValueIsNull) {
    switch (f.type) {
    case TType.BOOL:
    ...
    case TType.ENUM:
      ...
      return nullable(getSchema(enumMeta.enumClass), defaultValueIsNull);
    case TType.LIST:
      ListMetaData listMeta = (ListMetaData) f;
      return nullable(Schema.createArray(getSchema(listMeta.elemMetaData)), defaultValueIsNull);
    case TType.MAP:
  ...
  private Schema nullable(Schema schema) {
    return nullable(schema, true);
  }

  private Schema nullable(Schema schema, boolean defaultValueIsNull) {
