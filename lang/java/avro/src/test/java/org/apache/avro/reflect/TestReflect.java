...
public class TestReflect {
  ...
  void unionWithCollection() {
    Schema s = new SchemaParser().parse("[\"null\", {\"type\":\"array\",\"items\":\"float\"}]");
  ...
  @Test
  void unionWithMap() {
    Schema s = new SchemaParser().parse("[\"null\", {\"type\":\"map\",\"values\":\"float\"}]");
    GenericData data = ReflectData.get();
  ...
  @Test
  void unionWithFixed() {
    Schema s = new SchemaParser().parse("[\"null\", {\"type\":\"fixed\",\"name\":\"f\",\"size\":1}]");
    Schema f = new SchemaParser().parse("{\"type\":\"fixed\",\"name\":\"f\",\"size\":1}");
    GenericData data = ReflectData.get();
  ...
  @Test
  void unionWithEnum() {
    Schema s = new SchemaParser().parse("[\"null\", {\"type\":\"enum\",\"name\":\"E\",\"namespace\":"
  ...
  @Test
  void unionWithBytes() {
    Schema s = new SchemaParser().parse("[\"null\", \"bytes\"]");
    GenericData data = ReflectData.get();
  ...
  void r12() throws Exception {
    ...
    assertEquals(new SchemaParser().parse("{\"type\":\"array\",\"items\":[\"null\",\"string\"]}"),
