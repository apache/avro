...
abstract class AvroGradlePluginExtension @Inject constructor(objects: ObjectFactory) {
  ...
  /**
   * A list of zip files that contain Avro schema files. All generated Java classes are added to the classpath.
   *
   * <p>
  ...
  /**
   * The qualified names of classes which the plugin will look up, instantiate (through an empty constructor that must
   * exist) and set up to be injected into Velocity templates by Avro compiler.
   *
  ...
  /**
   * The createOptionalGetters parameter enables generating the getOptional... methods that return an Optional of the
   * requested type. This works ONLY on Java 8+
   *
  ...
  /**
   * The gettersReturnOptional parameter enables generating get... methods that return an Optional of the requested
   * type. This works ONLY on Java 8+
   *
  ...
  /**
   * The optionalGettersForNullableFieldsOnly parameter works in conjunction with gettersReturnOptional option. If it is
   * set, Optional getters will be generated only for fields that are nullable. If the field is mandatory, regular
   * getter will be generated. This works ONLY on Java 8+.
   *
  ...
  /**
   * If set to true, @Nullable and @NotNull annotations are added to fields of the record. The default is false. If
   * enabled, JetBrains annotations are used by default but other annotations can be specified via the
   * nullSafeAnnotationNullable and nullSafeAnnotationNotNull parameters.
   *
  ...
  /**
   * Controls which annotation should be added to nullable fields if createNullSafeAnnotations is enabled. The default
   * is org.jetbrains.annotations.Nullable.
   *
  ...
  /**
   * Controls which annotation should be added to non-nullable fields if createNullSafeAnnotations is enabled. The
   * default is org.jetbrains.annotations.NotNull.
   *
  ...
  /**
   * A set of fully qualified class names of custom {@link org.apache.avro.Conversion} implementations to add to the
   * compiler. The classes must be on the classpath at compile time and whenever the Java objects are serialized.
   *
  ...
  /**
   * A set of fully qualified class names of custom [org.apache.avro.LogicalTypes.LogicalTypeFactory] implementations to
   * add to the compiler. The classes must be on the classpath at compile time and whenever the Java objects are
   * serialized.
   *
