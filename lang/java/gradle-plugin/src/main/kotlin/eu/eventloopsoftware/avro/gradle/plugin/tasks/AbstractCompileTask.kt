...
abstract class AbstractCompileTask : DefaultTask() {
  ...
  @get:Input abstract val optionalGettersForNullableFieldsOnly: Property<Boolean>
  ...
  @get:InputFiles @get:Classpath abstract val runtimeClassPathFileCollection: ConfigurableFileCollection
      ...
      createClassLoader().use { classLoader ->
        customLogicalTypeFactories.get().forEach { factory ->
          try {
            ...
            val logicalTypeFactoryClass = classLoader.loadClass(factory) as Class<LogicalTypes.LogicalTypeFactory>
  ...
  private fun classPathFileCollection(): List<URL> = runtimeClassPathFileCollection.files.map { it.toURI().toURL() }
}
