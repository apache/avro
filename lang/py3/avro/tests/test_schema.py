...
DECIMAL_EXAMPLES = [
  # Valid decimal schema with bytes type
  ExampleSchema("""
    {
      "type": "bytes",
      "logicalType": "decimal",
  ...
  ExampleSchema("""
    {
      "type": "bytes",
      "logicalType": "decimal",
      "precision": "some string",
      "scale": 2
    }
    """,
    valid=False,
  ),
  # Invalid precision value (non integer)
  ExampleSchema("""
    {
      "type": "bytes",
      "logicalType": "decimal",
  ...
  # Invalid precision value (negative)
  ExampleSchema("""
    {
      "type": "bytes",
      "logicalType": "decimal",
  ...
  # Invalid scale value (non integer)
  ExampleSchema("""
    {
      "type": "bytes",
      "logicalType": "decimal",
  ...
  # Valid decimal schema with fixed type
  ExampleSchema("""
    {
      "type": "fixed",
      "logicalType": "decimal",
  ...
  # Invalid size value (too small for precision)
  ExampleSchema("""
    {
      "type": "fixed",
      "logicalType": "decimal",
...
LOGICAL_TYPE_EXAMPLES = DECIMAL_EXAMPLES + [
  ExampleSchema('{"type": "int", "logicalType": "date"}', True),
  ExampleSchema('{"type": "long", "logicalType": "date"}', False),

  ExampleSchema('{"type": "int", "logicalType": "time-millis"}', True),
  ExampleSchema('{"type": "long", "logicalType": "time-millis"}', False),

  ExampleSchema('{"type": "int", "logicalType": "time-micros"}', False),
  ExampleSchema('{"type": "long", "logicalType": "time-micros"}', True),

  ExampleSchema('{"type": "int", "logicalType": "timestamp-millis"}', False),
  ExampleSchema('{"type": "long", "logicalType": "timestamp-millis"}', True),

  ExampleSchema('{"type": "int", "logicalType": "timestamp-millis"}', False),
  ExampleSchema('{"type": "long", "logicalType": "timestamp-micros"}', True),
...
EXAMPLES += LOGICAL_TYPE_EXAMPLES
...
class TestSchema(unittest.TestCase):
  ...
  def testParseLogicalTypes(self):
    correct = 0
    for example in LOGICAL_TYPE_EXAMPLES:
      try:
        schema_data = schema.Parse(example.schema_string)
      except schema.AvroException:
        schema_data = None

      if example.valid == isinstance(schema_data, schema.LogicalTypeMixin):
        correct += 1
      else:
        self.fail('Validity check failed: %r' % example.schema_string)

    self.assertEqual(correct, len(LOGICAL_TYPE_EXAMPLES),
      'Parse logical types behavior correct on %d out of %d schemas.'
      % (correct, len(EXAMPLES)),
    )
