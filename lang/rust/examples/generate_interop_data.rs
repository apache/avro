...
use avro_rs::{schema::Schema, types::{Record, Value}, Writer, Codec};
...
fn main() -> anyhow::Result<()> {
    ...
    datum.put("arrayField", Value::Array(vec![
        Value::Double(5.0),
        Value::Double(0.0),
        Value::Double(12.0),
    ]));
    let mut map = HashMap::new();
    map.insert("a".into(), Value::Record(vec![
        ("label".into(), Value::String("a".into())),
    ]));
    map.insert("bee".into(), Value::Record(vec![
        ("label".into(), Value::String("cee".into())),
    ]));
    ...
    datum.put("recordField",Value::Record(vec![
    ...
    for codec in Codec::iter() {
        let out = Vec::new();
        let mut writer = Writer::with_codec(&schema, out, codec);
        writer.append(datum.clone())?;
        let out2 = writer.into_inner()?;

        let suffix = if codec.name() == "null" {
            "".to_owned()
        } else {
            format!("_{}", codec.name())
        };

        std::fs::write(format!("../../build/interop/data/rust{}.avro", suffix), out2)?;
    }
