...
DataFileWriterBase::DataFileWriterBase(const char* filename,
		...
		filename_(filename), schema_(schema), encoderPtr_(binaryEncoder()), syncInterval_(
				syncInterval), codec_(codec), stream_(
				fileOutputStream(filename)), buffer_(memoryOutputStream()), sync_(
				makeSync()), objectCount_(0) {
	if (syncInterval < minSyncInterval || syncInterval > maxSyncInterval) {
		throw Exception(
				boost::format("Invalid sync interval: %1%. "
...
DataFileReaderBase::DataFileReaderBase(const char* filename) :
		filename_(filename), stream_(fileInputStream(filename)), decoder_(
				binaryDecoder()), objectCount_(0), eof_(false) {
	readHeader();
}
...
void DataFileReaderBase::init(const ValidSchema& readerSchema) {
	...
	dataDecoder_ =
			(toString(readerSchema_) != toString(dataSchema_)) ?
					resolvingDecoder(dataSchema_, readerSchema_,
							binaryDecoder()) :
					binaryDecoder();
	readDataBlock();
}
...
bool DataFileReaderBase::readDataBlock() {
	...
	if (codec_ == NULL_CODEC) {
		...
		os_->push(
				boost::iostreams::basic_array_source<char>(&compressed_[0],
						compressed_.size()));
		...
		if (checksum != crc()) {
			throw Exception("Checksum did not match for Snappy compression");
		}
		os_.reset(new boost::iostreams::filtering_istream());
		os_->push(
...
void DataFileReaderBase::readHeader() {
	...
	if (magic != m) {
		throw Exception(
				"Invalid data file. Magic does not match: " + filename_);
	}
	...
	if (it != metadata_.end() && toString(it->second) == AVRO_DEFLATE_CODEC) {
		...
		if (it != metadata_.end() && toString(it->second) != AVRO_NULL_CODEC) {
			throw Exception(
					"Unknown codec in data file: " + toString(it->second));
		}
	}
