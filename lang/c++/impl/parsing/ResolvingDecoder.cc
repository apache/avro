...
class ResolvingGrammarGenerator : public ValidatingGrammarGenerator {
    ...
    static size_t bestBranch(const NodePtr &writer, const NodePtr &reader);
...
size_t ResolvingGrammarGenerator::bestBranch(const NodePtr &writer,
...
ProductionPtr ResolvingGrammarGenerator::doGenerate2(
    ...
    if (writerType == readerType) {
        ...
        switch (readerType) {
            ...
            case AVRO_UNION: {
                size_t j = bestBranch(writer, reader);
                if (j >= 0) {
...
float ResolvingDecoderImpl<P>::decodeFloat() {
    ...
    return k == Symbol::Kind::Int ? (float) base_->decodeInt() : k == Symbol::Kind::Long ? (float) base_->decodeLong() : base_->decodeFloat();
}

template<typename P>
double ResolvingDecoderImpl<P>::decodeDouble() {
    Symbol::Kind k = parser_.advance(Symbol::Kind::Double);
    return k == Symbol::Kind::Int ? (double) base_->decodeInt() : k == Symbol::Kind::Long ? (double) base_->decodeLong() : k == Symbol::Kind::Float ? (double) base_->decodeFloat() : base_->decodeDouble();
}
