...
namespace Avro.Reflect
{
    ...
    public class ReflectDefaultReader : SpecificDefaultReader
    {
        ...
        public ReflectDefaultReader(Type objType, Schema writerSchema, Schema readerSchema, IReflectCache cache)
            : base(writerSchema, readerSchema)
        {
            _reflectCache = cache;
            _reflectCache.LoadClassCache(objType, readerSchema);
        }
        ...
        internal Type GetTypeFromSchema(Schema schema, bool nullable)
        {
            switch (schema.Tag)
            {
                ...
                case Schema.Type.Enumeration:
                    ...
                    Type enumType = null;
                    enumType = _reflectCache.GetEnumeration(namedSchema);
                    if (enumType == null)
                    {
                ...
                case Schema.Type.Error:
                    ...
                    recordtype = _reflectCache.GetClass(recordSchema).GetClassType();
                    if (recordtype == null)
                    {
                        throw new Exception(string.Format(CultureInfo.InvariantCulture,
                ...
                case Schema.Type.Array:
                    ...
                    var arrayHelper = _reflectCache.GetArrayHelper(arraySchema, null);
        ...
        public object GetDefaultValue(Schema s, JToken defaultValue)
        {
            ...
            switch (s.Tag)
            {
                ...
                case Schema.Type.Array:
                    ...
                    var arrayHelper = _reflectCache.GetArrayHelper(s as ArraySchema, array);
                ...
                case Schema.Type.Error:
                    ...
                    foreach (Field field in rcs)
                    {
                        ...
                        _reflectCache.GetClass(rcs).SetValue(rec, field, GetDefaultValue(field.Schema, val));
                    }
        ...
        protected override object ReadEnum(object reuse, EnumSchema writerSchema, Schema readerSchema, Decoder d)
        {
            ...
            var enumType = _reflectCache.GetEnumeration(es);
        ...
        protected override object ReadRecord(object reuse, RecordSchema writerSchema, Schema readerSchema, Decoder dec)
        {
            ...
            foreach (Field wf in writerSchema)
            {
                try
                {
                    ...
                    if (rs.TryGetField(wf.Name, out rf))
                    {
                        ...
                        _reflectCache.GetClass(writerSchema).SetValue(rec, rf, Read(obj, wf.Schema, rf.Schema, dec));
                    }
                    else
                    {
                        Skip(wf.Schema, dec);
            ...
            foreach (Field rf in rs)
            {
                ...
                _reflectCache.GetClass(rs).SetValue(rec, rf, GetDefaultValue(rf.Schema, rf.DefaultValue));
            }
        ...
        protected override object ReadArray(object reuse, ArraySchema writerSchema, Schema readerSchema, Decoder dec)
        {
            ...
            if (reuse != null)
            {
                ...
                arrayHelper = _reflectCache.GetArrayHelper(rs, array);
