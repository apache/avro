...
namespace Avro.Generic
{
    ...
    public class DefaultReader
    {
        ...
        public object Read(object reuse, Schema writerSchema, Schema readerSchema, Decoder decoder)
        {
            ...
            switch (writerSchema.Tag)
            {
                ...
                case Schema.Type.Boolean:
                    return Read<bool>(writerSchema.Tag, readerSchema, decoder.ReadBoolean);

                case Schema.Type.Int:
                    {
                        int i = Read<int>(writerSchema.Tag, readerSchema, decoder.ReadInt);
                ...
                case Schema.Type.Long:
                    {
                        long l = Read<long>(writerSchema.Tag, readerSchema, decoder.ReadLong);
                        switch (readerSchema.Tag)
                        {
                            case Schema.Type.Float:
                                return (float)l;
                ...
                case Schema.Type.Float:
                    {
                        float f = Read<float>(writerSchema.Tag, readerSchema, decoder.ReadFloat);
                        switch (readerSchema.Tag)
                        {
                            case Schema.Type.Double:
                                return (double)f;
                ...
                case Schema.Type.Double:
                    return Read<double>(writerSchema.Tag, readerSchema, decoder.ReadDouble);

                case Schema.Type.String:
                    return Read<string>(writerSchema.Tag, readerSchema, decoder.ReadString);

                case Schema.Type.Bytes:
                    return Read<byte[]>(writerSchema.Tag, readerSchema, decoder.ReadBytes);
        ...
        /// </returns>
        /// <exception cref="AvroException">{ex.Message} in field {wf.Name}</exception>
        protected virtual object ReadRecord(object reuse, RecordSchema writerSchema, Schema readerSchema, Decoder decoder)
        {
            RecordSchema recodSchema = (RecordSchema)readerSchema;

            object record = CreateRecord(reuse, recodSchema);
            foreach (Field writerField in writerSchema)
            {
                try
                {
                    if (recodSchema.TryGetFieldAlias(writerField.Name, out Field recordField))
                    {
                        TryGetField(record, writerField.Name, recordField.Pos, out object obj);
                        AddField(record, writerField.Name, recordField.Pos, Read(obj, writerField.Schema, recordField.Schema, decoder));
                    }
                    else
                    {
            ...
            foreach (Field rf in recodSchema)
            {
                ...
                TryGetField(record, rf.Name, rf.Pos, out object obj);
                AddField(record, rf.Name, rf.Pos, Read(obj, rf.Schema, rf.Schema, defaultDecoder));
            }
        ...
        {
            GenericRecord genericRecord = (reuse == null || !(reuse is GenericRecord) || !(reuse as GenericRecord).Schema.Equals(readerSchema)) ?
        ...
        protected virtual object CreateEnum(object reuse, EnumSchema es, string symbol)
        {
            if (reuse is GenericEnum)
            {
                ...
                if (genericEnum.Schema.Equals(es))
                {
        ...
        {
            return (reuse != null && reuse is object[]) ? reuse : Array.Empty<object>();
        }
        ...
        protected virtual object ReadUnion(object reuse, UnionSchema writerSchema, Schema readerSchema, Decoder decoder)
        {
            ...
            Schema wSchema = writerSchema[index];

            if (!readerSchema.CanRead(wSchema))
            {
                throw new AvroException($"Schema mismatch. Reader: {ReaderSchema}, writer: {WriterSchema}");
            }
            ...
            {
                readerSchema = findBranch(readerSchema as UnionSchema, wSchema);
            }

            return Read(reuse, wSchema, readerSchema, decoder);
        }
        ...
        /// </returns>
        /// <exception cref="AvroException">No matching schema for {schema} in {unionSchema}</exception>
        protected static Schema findBranch(UnionSchema unionSchema, Schema schema)
        {
            int index = unionSchema.MatchingBranch(schema);
            return index >= 0 ? unionSchema[index] :
