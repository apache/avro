...
Avro implementations for C, C++, C#, Java, PHP, Python, and Ruby can be downloaded from the [Apache Avro™ Releases]({{< relref "/project/download" >}}) page. This guide uses Avro 1.11.0, the latest version at the time of writing. For the examples in this guide, download avro-1.11.0.jar and avro-tools-1.11.0.jar.
...
Avro schemas are defined using JSON. Schemas are composed of [primitive types](https://avro.apache.org/docs/1.11.0/spec.html#schema_primitive) (null, boolean, int, long, float, double, bytes, and string) and [complex types](https://avro.apache.org/docs/1.11.0/spec.html#schema_complex) (record, enum, array, map, union, and fixed). You can learn more about Avro schemas and types from the specification, but for now let's start with a simple schema example, user.avsc:
